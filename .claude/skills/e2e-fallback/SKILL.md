...
- qa-e2e 收到證據後，**先做下方〈儀器來源查核〉**（兩個判準都過才有東西可判），再開始判讀。
- qa-e2e 依原輸出契約出 Verdict，其中：
  - 「流程逐步結果」每一步要寫**畫面實際字面**（逐字，不要改寫、不要意譯）。
  - 「視覺抽查」寫量測到的實際值 vs 規範值。
  - 任何**沒能實機看到**的項目寫「無法驗收」＋原因，不得用 code 推測補位。

### 儀器來源查核（qa-e2e 收到證據後的第一件事）

**這裡有兩個獨立判準，缺一不可**——只做判準 2 而基準取錯，查核會自洽通過、形同虛設。
任一判準不過 → **該批證據作廢**，不得據以判斷，走第 7 節。

0. **先看 stdout 開頭**：若以 `e2e-fallback: ABORT` 開頭，代表 runner 的斷言已擋下該次執行，
   **根本沒有證據可判**，直接走第 7 節，不必往下查。
1. **判準一・比對基準必須外部取得**：比對用的前綴一律取自**第 2 節的既定前綴**
   （devops-sre 的環境定義），**不得以 stdout 裡的 `provenance.trustedPrefix` 為準**。
   若 `provenance.trustedPrefix` 與既定前綴不一致 → 該次執行**放寬了信任邊界**，證據作廢。
   （拿封包自己宣告的基準去量封包自己的路徑必然通過；極端情況 `TRUSTED_PREFIX=/` 會一路綠燈。）
2. **判準二・兩個路徑都要在前綴內**：`provenance.playwrightModule` 與
   `provenance.chromiumExecutable`，任一落在前綴之外
   （尤其是指進受測專案的 `node_modules`）→ 證據作廢。

> **為什麼這條由裁判查、不由代跑者自證**：第 2 節的探測與 runner 的 `require()` 是
> **兩次獨立的解析事件**，探測綠燈不代表這次跑用的是同一份——**量測儀器不得由受量測者提供**。
...
> 現在 runner 開頭的 fail-closed 斷言會讓這種跑法**直接 `exit 1`**；
> 本查核是它的第二道稽核（防止有人改了骨架、拿掉斷言、或用舊版腳本），不是唯一防線。
