...
- **2026-07-28**：**台股資料源首次真實環境驗證通過**。CEO 於本機（有真實網路）以 Docker Compose 跑起全套，確認台股日線可實際抓取並在 UI 呈現。此為 TWSE/TPEx/FinMind adapter 自離線開發以來第一次接觸真實 API。
  範圍限定（誠實記錄）：(a) 單次人工測試，非自動化契約驗證；(b) 實際供應資料的是主來源或備援未確認（UI 來源徽章可查，未回報）；(c) 本次驗證的是「端點格式與解析可用」，**不改變**以下仍未查證項目：回測費率（`verified_on=null`）、槓桿 ETF 倍數與費用率（`verified=false`）、FX 端點與中點模型、Alpha Vantage / yfinance 美股與指數端點（CEO 未設 AV key，未測 US 路徑）。
