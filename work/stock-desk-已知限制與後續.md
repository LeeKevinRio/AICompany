...
- **2026-08-12（續,除權息同步真實驗證通過,CEO 本機執行,附終端輸出證據）**：
  `app.dividends.sync` PASS——寫入 108 筆除權息預告事件,另 16 列依防禦規則拒收並計數揭露
  (略過原因分佈待覆核)。過程修正:原推測端點 `TWT49U` 實測為死路(回 HTML 且不在官方
  swagger 目錄),CEO 以 swagger 探測+真實樣本確認正解為 `exchangeReport/TWT48U_ALL`
  (除權除息預告表),data-engineer 據實測樣本重寫解析(commit `0c4e683`)。
