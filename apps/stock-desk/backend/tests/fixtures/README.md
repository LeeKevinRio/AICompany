...
| `twse_openapi_twt48u_all.json` | TWSE OpenAPI `v1/exchangeReport/TWT48U_ALL`（上市股票除權除息預告表，`app/dividends/providers.py` 除權息還原用） | **端點路徑與欄位結構已由 CEO 2026-08-12 實測確認**（`TWT49U` 已確認不存在、`t187ap45_L` 已評估排除，見 `app/dividends/providers.py` 檔頭）；`Name` 欄位值為合成佔位，僅欄位「結構」對齊實測樣本。合成內容含：一列純現金除息（無 `StockDividendRatio`）、一列全數值欄位皆為空字串（僅 `Exdividend="息"` 這個真實樣本會出現的型態）、一列 `Exdividend="息權"` 且 `StockDividendRatio` 有值（測「記錄存在但不計算」分支）、一列 `Exdividend` 為未知值（測防禦分支，計入 skip）、一列缺 `Code`（測跳過邏輯）、一列 `Exdividend="權"` 但 `StockDividendRatio` 空白（測「旗標與欄位不一致」異常，計入 skip）、一列 `CashDividend` 含千分位逗號 |
