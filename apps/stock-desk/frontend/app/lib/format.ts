 ...
 *
 * **狀態：risk-compliance-officer 三輪定稿（2026-08-16）**——初稿由 dev-lead 自擬
 * 遭 VETO，二、三輪由 creative-lead 改寫後逐字核可（歷程見
 * `work/stock-desk-D4-資料來源措辭.md` 第五節）。字面（含標點與分號位置）不得再
 * 改動，任何變更視為漂移須重送風控；`directorySearch.test.ts` 的逐字守門測試
 * 同步比對本字串。
 *
 * 寫法約束（依 CEO 指示與風控裁決）：只陳述事實，不得出現「自動幫你分類好了
 * 不用管」類語氣，不得暗示預設值比使用者自己選的更正確；刻意**不**標示個別值是
 * 自動帶入還是手填（同一封閉清單、同源官方資料，區分只會製造無意義差別，且標記
 * 會誘導使用者對其中一類降低審視）。留空的當下後果以現在式否定式陳述（該持倉的
 * 產業佔比上限如實顯示無法評估），不得與「下次同步會補上」串成因果鏈——同步是
 * 手動 CLI，可能長期不發生。
 *
 * 情境約束（風控三輪 suggested ③）：本句省略了「該部位的」限定語，僅因它只在
 * 單一持倉表單（`form.market === "TW"`）內渲染、就地情境足以消歧;若日後搬到
 * 帳本層或設定頁等非單一持倉情境顯示，須補回「該部位的」並重新送審。
 */
