...
/**
 * 產業別欄位旁的來源說明（CEO 指示 2026-08-16 第 4 點）。
 *
 * **狀態：待 risk-compliance-officer 覆核**——本句由 dev-lead 自擬，尚未經風控
 * 核可；覆核前不得視為定稿，覆核後若有改寫須同步更新此註解與
 * `directorySearch.test.ts` 的逐字守門測試。
 *
 * 寫法約束（依 CEO 指示）：只陳述事實（資料來自哪裡、什麼時候的、哪些標的沒有、
 * 使用者可以改），不得出現「自動幫你分類好了不用管」這類要使用者放手的語氣，也
 * 不得暗示這個預設值比使用者自己選的更正確。刻意**不**標示個別值是自動帶入還是
 * 手填：兩者都是同一份封閉清單裡的值、事實層面同源官方資料，在畫面上區分只會製造
 * 一個沒有意義的差別。
 *
 * 「同步狀態可在設定頁的『證券目錄』區塊查看」指的是 `DirectorySection.tsx` 實際
 * render 的已同步／尚未同步徽章——該區塊確實存在且確實顯示狀態。它顯示的時間戳是
 * 查詢時間而非同步時間（見該檔註解），所以這裡只稱「狀態」，不稱「時間」。
 */
export const SECTOR_SOURCE_DISCLOSURE =
  "產業別的預設值取自證交所公開資料中的上市公司產業別，隨本機證券目錄同步一併更新；" +
  "其新舊取決於證券目錄最後一次同步的時間，同步狀態可在設定頁的「證券目錄」區塊查看。" +
  "上櫃股票與 ETF 不在該來源的涵蓋範圍，這些標的會留空。" +
  "本欄位可自行修改，儲存的是畫面上顯示的值。";
