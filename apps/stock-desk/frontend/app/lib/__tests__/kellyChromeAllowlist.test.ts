 ...
 * .tsx` under a three-part test (chrome 判準): (i) verbatim-or-subject-only
...
const ALLOWLIST: Record<string, readonly string[]> = {
  ...
  "../../settings/KellyImportDialog.tsx": [
    // L194/L212/L230/L244 — the mini backtest-spec form's own field labels,
    // 逐字同構 BacktestForm.tsx's own #bt-strategy/#bt-instrument/#bt-start/
    // #bt-end labels (same four words, same fields, same convention).
    "策略",
    "類型",
    "開始日期",
    "結束日期",
    // L362 — generic error fallback, 逐字同構 `ErrorPanel.tsx`/every other
    // `error instanceof ApiError ? error.message : "未知錯誤"` call site in
    // this app (12+ 慣例出處 per 條件 106's own count). 條件 106 explicitly
    // named this line as the one chrome exemption that must be recorded
