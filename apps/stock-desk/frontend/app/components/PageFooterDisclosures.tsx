 ...
 * - STANDING: never collapsible or clipped (風控 L2 lists the forbidden
 *   constructs; the scan test enforces them), never conditional on any
 *   query state for the static groups (資料來源 is a constant; the other
 *   groups appear when their section's data exists).
 ...
 *   together with the L2 no-collapse scan.
 */
...
export function PageFooterDisclosures({ groups }: { groups: readonly FooterGroup[] }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-xs text-neutral-400">{PAGE_FOOTER_DISCLOSURES_INTRO}</p>
